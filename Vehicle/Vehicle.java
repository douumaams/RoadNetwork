...
public class Vehicle implements NetworkElement
{
    ...
    // méthode qui déplace la voiture d'une seule case
    public void elementaryMove()
    {
        ...
        {
            System.out.println("Il faudra gérer ça avec les jonctions 1");
        }
        else
        {
            if ((this.position + 1 < b.length) && !b[this.position + 1].hasVehicle()) // pos suivante
            {
                ...
                if (this.position >= b.length)
                {
                    System.out.println("Il faudra gérer ça avec les jonctions 2");
                }
                else
                {
